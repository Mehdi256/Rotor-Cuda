...
#define FILTER_SIZE 8
	...
	while (isAlive(params)) {
		...
		if (rKey > 0) {
			if (isAlive(params)) {
				double avgKeyRate2 = avgKeyRate * 2;
			...
			if (avgGpuKeyRate > 1000000000) {
				if (isAlive(params)) {
					memset(timeStr, '\0', 256);
					printf("\r  [%s] [%s] [CPU+GPU: %.2f Gk/s] [GPU: %.2f Gk/s] [C: %lf %%] [T: %s (%d bit)] [F: %d]   ",
						toTimeStr(t1, timeStr),
						rhex.GetBase16().c_str(),
						avgKeyRate / 1000000000.0,
						avgGpuKeyRate / 1000000000.0,
				...
				if (isAlive(params)) {
					memset(timeStr, '\0', 256);
					printf("\r  [%s] [%s] [CPU+GPU: %.2f Mk/s] [GPU: %.2f Mk/s] [C: %lf %%] [T: %s (%d bit)] [F: %d]   ",
						toTimeStr(t1, timeStr),
						rhex.GetBase16().c_str(),
						avgKeyRate / 1000000.0,
						avgGpuKeyRate / 1000000.0,
